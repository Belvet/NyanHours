...
<div class="timesheet-heading"><div><h1>Planilla semanal</h1><p class="muted">Horas de <?= e($user['name']) ?></p></div><?php if(!$rangeMode):?><div class="week-nav"><a href="?week=<?= e($weekStart->modify('-7 days')->format('Y-m-d')) ?>">‹</a><span><?= e($weekStart->format('d/m')) ?> – <?= e($weekEnd->format('d/m/Y')) ?></span><a href="?week=<?= e($weekStart->modify('+7 days')->format('Y-m-d')) ?>">›</a></div><?php endif;?></div>
...
<?php if($rangeMode):?><p class="filter-summary">Período seleccionado: <strong><?=e(date('d/m/Y',strtotime($dateFrom)))?></strong> — <strong><?=e(date('d/m/Y',strtotime($dateTo)))?></strong></p><?php endif;?>
