...
<?php if($filterActive):?><p class="filter-summary">Período seleccionado: <strong><time datetime="<?=e($dateFrom)?>" data-local-date="<?=e($dateFrom)?>"><?=e(date('d/m/Y',strtotime($dateFrom)))?></time></strong> — <strong><time datetime="<?=e($dateTo)?>" data-local-date="<?=e($dateTo)?>"><?=e(date('d/m/Y',strtotime($dateTo)))?></time></strong></p><?php endif;?>
...
<?php foreach($clientsReport as $reportClientId=>$client): $exportQuery=['client_id'=>$reportClientId]; if($filterActive){$exportQuery['date_from']=$dateFrom;$exportQuery['date_to']=$dateTo;} ?><section class="person-report"><header><h2 class="client-name" style="--client-color:<?=e($client['color'])?>"><i></i><?=e($client['name'])?></h2><div class="report-client-actions"><strong><?=e(formatMinutes((int)$client['total']))?></strong><a class="button-secondary report-export" href="/admin/export-report.php?<?=e(http_build_query($exportQuery))?>">Exportar horas</a><a class="button-secondary invoice-button" href="/admin/invoice.php?<?=e(http_build_query($exportQuery))?>">Crear invoice</a></div></header>
